...
db.collection('post').updateOne({ _id : parseInt(요청.body.id) }, { $set : { 제목 : 요청.body.title, 내용 : 요청.body.contents }}, function(에러, 결과){ 
    console.log('수정완료')
    응답.redirect('/list')
})
...
app.post('/register', function(요청, 응답){
  db.collection('login').insertOne({ nickname : 요청.body.nickname, id : 요청.body.id, pw : 요청.body.pw}, function(에러, 결과){
      응답.redirect('/')
  } )
})
...
app.post('/add', function(요청, 응답){
  응답.redirect('/list')
    db.collection('counter').findOne({name : '게시물갯수'}, function(에러, 결과){
        ...
        var 총게시물갯수 = 결과.totalPost;
        var 저장할거 = { _id : 총게시물갯수 + 1, 작성자 : 요청.user._id, 제목 : 요청.body.title, 내용 : 요청.body.contents, 날짜 : 요청.body.date}
        db.collection('post').insertOne(저장할거, function(에러, 결과){
            console.log('저장완료');
            db.collection('counter').updateOne({name:'게시물갯수'},{ $inc : {totalPost:1}})
        });  
...
app.post('/chatroom', function(요청, 응답){
  ...
  db.collection('chatroom').find({ member : 요청.user._id }).toArray(function(에러, 결과){
    var userCheck = JSON.stringify(결과);
    if(userCheck.indexOf(요청.body.targetUser) > -1 ){
      응답.write("<script>alert('chatroom')</script>")
    }else{
      var 저장할거 = {
        title : '채팅방',
...
/*  수정중임
app.get('/commu', 로그인했니, function(요청, 응답){ 

  db.collection('login').find({ member : 요청.user._id }).toArray().then((결과)=>{
    console.log(결과);
    응답.render('commu.ejs', {data : 결과})
  })
